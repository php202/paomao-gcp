 ...
 * 
 * Usage:
 *   NODE_PATH=~/泡泡貓/dashboard/node_modules node ~/paomao-gcp/gcp/scripts/monthly_service_fee.cjs
 *   --dry-run    只顯示不寫入
...
async function main() {
  ...
  const description = `${month.replace('-', '-')}月服務費`;

  console.log(`[service-fee] 🚀 開始 (month=${month}, dry_run=${DRY_RUN})`);

  const pool = new Pool({ database: 'paomao' });

  // 1. Get stores with service_fee (DISTINCT ON store to avoid duplicate payees)
  const { rows: stores } = await pool.query(`
    SELECT DISTINCT ON (s.id) s.id as store_id, s.store_name, s.service_fee, s.company, s.tax_id,
           p.code as payee_code, p.id as payee_id
    FROM stores s
    LEFT JOIN payee_stores ps ON ps.store_id = s.id
    LEFT JOIN payees p ON p.id = ps.payee_id
    WHERE s.service_fee > 0 AND s.store_name NOT ILIKE '%停止%'
    ORDER BY s.id, p.id
  `);

  console.log(`[service-fee] 找到 ${stores.length} 間有服務費的店`);
  if (!stores.length) { await pool.end(); return; }

  // 2. Check duplicates in DB
  const { rows: existingRows } = await pool.query(
    `SELECT store_name FROM ach_records WHERE fee_type = '服務費' AND description = $1`,
    [description]
  );
  ...
  for (const store of stores) {
    ...
    const amount = parseFloat(store.service_fee);
    ...
    try {
      ...
      // B. Append to Sheet
      const newRow = [dateStr, shortName, amount, store.payee_code || '', '服務費', description, '', '', '', '', '', '', '', '', '', odooOrderName || '', ''];
      ...
      await pool.query(`
        ...
        VALUES ($1, $2, $3, $4, $5, '服務費', $6, $7, $8, $9, 2026)
  ...
  console.log(`[service-fee] 🏁 完成：新增 ${created} 筆${DRY_RUN ? ' (dry-run)' : ''}`);
  await pool.end();
}

main().catch(e => { console.error('[service-fee] Fatal:', e); process.exit(1); });
