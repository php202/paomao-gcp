...
async function getDirectStoreReplyStatusTextAction(auth) {
  if (!LINE_STORE_SS_ID) return { ok: false, message: '無法取得店家回覆狀態，請稍後再試。' };
  const stores = await readSheet(auth, LINE_STORE_SS_ID, "'店家基本資料'!A:L");
  const msgs = await readSheet(auth, LINE_STORE_SS_ID, "'訊息一覽'!A:F");
  const directStores = [];
  ...
  if (!directStores.length) return { ok: false, message: '目前無直營店資料或 H 欄皆為 false' };
