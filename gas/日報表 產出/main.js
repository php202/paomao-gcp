...
function runAccNeed(options) {
  options = options || {};
  // 日報改由 GCP 產出（不依賴 Core API getCoreConfig / getLineSayDouInfoMap）
  const { url: gcpAdminUrl, key: gcpAdminKey, useAdmin } = getGcpAdminParams_();
  if (!useAdmin) {
    throw new Error(
      '日報已改由 GCP 產出。請在「日報表 產出」專案「專案設定」→「指令碼屬性」新增：\n' +
      '• GCP_ADMIN_URL = Cloud Run 服務網址 + /admin（例：https://pao-checkin-api-xxx.run.app/admin）\n' +
      '• GCP_ADMIN_KEY = 與 ADMIN_KEY / PAO_CAT_SECRET_KEY 相同\n' +
      '排程觸發時會呼叫此 GCP 端點產出日報。'
    );
  }
  const adminRes = callGcpAdmin_('runDailyReport', options);
  if (adminRes && adminRes.status === 'ok') {
    if (typeof SpreadsheetApp.getUi === 'function') {
      SpreadsheetApp.getUi().alert('已送出 GCP 日報工作（請至 Cloud Run Logs 查看進度）');
    } else {
      console.log('GCP 日報已送出');
    }
    return;
  }
  if (adminRes && adminRes.status === 'error') {
    throw new Error('GCP 日報執行失敗：' + (adminRes.message || 'unknown'));
  }
  throw new Error('無法連線至 GCP（請確認 GCP_ADMIN_URL 是否正確、Cloud Run 服務是否運行）');
}
